...
- feature flags, Free/Pro entitlements, contextual paywall;
- локалізація `uk` + `en`, частковий `pl` (домашній екран і зони) з per-key
  fallback, усе з одного authoring-джерела;
- 142 тести (state machine, timeline, контент, локалізація, entitlements,
...
Web-збірка того ж коду, для перегляду UI та флоу. Це **не** реліз і не
медична рекомендація: дозування, safety-тексти й пози не пройшли клінічного
рев'ю (`docs/SAFETY_AND_CLINICAL_REVIEW.md`). Кожна така вправа каже про це
на своєму екрані.
